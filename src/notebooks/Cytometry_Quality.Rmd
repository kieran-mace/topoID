...
---
@TODO modify analysis to look at expression and event count. 
```{r, message=FALSE}
library(dplyr)
library(stringr)
library(ggplot2)
...
meta = meta %>% 
    mutate(fitc = Vectorize(get_summary_statistics)(pathway_path) + 1, num = Vectorize(get_num)(pathway_path)) #%>%
...
```{r}
meta %>% ggplot(aes(x=Column,y=Row, fill = num)) + geom_tile() + facet_grid(Repressor~Activator)
```
